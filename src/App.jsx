...
const HEB_SAY = {
  ...
  "בספרת": "בְּסִפְרַת", "מעבר": "מַעֲבָר", "ביחידות": "בַּיְּחִידוֹת",
  ...
  "ירוקים": "יְרוּקִּים", "כש": "כְּשֶׁ", "הניצבים": "הַנִּיצָּבִים", "וניצב": "וְנִיצָּב",
  "במאונך": "בִּמְאוּנָּךְ", "הפירוק": "הַפֵּירוּק", "פירוש": "פֵּירוּשׁ", "משותף": "מְשׁוּתָּף",
  ...
  "מהאדמה": "מֵהָאֲדָמָה", "ריק": "רֵיק", "ולילה": "וָלַיְלָה", "וו": "וָו", "צחצח": "צִחְצַח",
  "מכוונים": "מְכוּוָנִים",
  "מיישרים": "מְיַישְּׁרִים", "כתום": "כָּתוֹם",
};
const HEB_SAY_RE = new RegExp(`(^|[^א-ת])(${Object.keys(HEB_SAY).join("|")})(?![א-ת])`, "g");
const sayNiqqud = (t) => t.replace(HEB_SAY_RE, (m, pre, w) => pre + HEB_SAY[w]);

const SAY_FIX = [
  // "חסרה" אפשר לקרוא בשתי דרכים (חָסְרָה / חֲסֵרָה), ו"החסרה" נשמע כמו הַחְסָרָה.
  // ניקוד וכתיב מלא לא עוזרים כאן (נבדק מול הקריין), לכן בהקראה בלבד אומרים "נעלמה" —
  // על המסך נשאר הכיתוב המוכר מבית הספר.
  [/([^א-ת]|^)החסרה(?![א-ת])/g, "$1שנעלמה"],
  [/([^א-ת]|^)חסרה(?![א-ת])/g, "$1נעלמה"],
...
function speakHe(text, opts = {}) {
  ...
  // מסירים ניקוד שמגיע מהתוכן (לא אחיד), ומזינים במקומו ניקוד בדוק (sayNiqqud) למילים שנקראו לא נכון
  let t = String(text);
